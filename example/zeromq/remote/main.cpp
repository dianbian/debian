...
void onConnection(LengthHeaderCodec* codec, const TcpConnectionPtr& conn)
{
  if (conn->connected())
  {
    LOG_INFO << "connected";
                     ...
                     Timestamp)
{
  ...
  else
  {
    ...
    LOG_INFO << Fmt("%.3f", g_msgCount / elapsed) << " round-trips per second";
    LOG_INFO << Fmt("%.3f", (1000000 * elapsed / g_msgCount / 2))
             << " latency [us]";
    LOG_INFO << Fmt("%.3f", (g_msgSize * g_msgCount / elapsed / 1024 / 1024))
             << " band width [MiB/s]";
    conn->shutdown();
  }
}
...
int main(int argc, char* argv[])
{
  if (argc > 3)
  {
    ...
    client.setConnectionCallback(std::bind(onConnection, &codec, std::placeholders::_1));
